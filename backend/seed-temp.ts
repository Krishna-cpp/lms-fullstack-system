/**
 * Seed script — run with: npx ts-node seed-temp.ts
...
// Load .env from backend directory (current dir)
...
async function seed() {
    ...
    try {
        ...
        for (const c of courses) {
            // Check if course exists first to avoid duplicate insertions if running multiple times without cleanup
            // Actually INSERT IGNORE doesn't work well with auto-increment IDs if we want to capture them.
            // But for now let's just insert standard. If duplicate, it fails, but we catch error.
            // Using INSERT IGNORE on courses table? Schema has unique constraints only on ID?
            // No, schema doesn't have unique constraint on title. So duplicates possible.
            // But let's assume empty DB or just run it.
